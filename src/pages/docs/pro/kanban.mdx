...
export const Usage = () => {
    ...
    return (<View width="100%" height={700}>
            ...
            <MenuProvider menu={getMenu}>
                <Kanban style={{ '--f-kanban-swimlane-minheight': '100%' }} swimlanes={swimlanes} onCardOpen={handleCardOpen} onCardAdd={handleCardAdd} onCardMove={handleCardMove} onColumnAdd={handleColumnAdd} onColumnMove={handleColumnMove} onSwimlaneMove={handleSwimlaneMove} onColumnUpdate={handleColumnUpdate} onSwimlaneUpdate={handleSwimlaneUpdate} hideCheckbox={false} collapsibleColumn={true} targetVariant={{ cards: 'animated', nav: 'focus' }} card={undefined} columnHeader={undefined} swimlaneHeader={undefined}/>
            </MenuProvider>
        </View>);
};
